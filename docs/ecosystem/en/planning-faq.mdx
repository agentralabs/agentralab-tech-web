...
All 13 MCP tools become available to Claude. See [Playbooks](/docs/en/planning-playbooks-agent-integration) for system prompt templates.
...
See [Benchmarks](/docs/en/planning-benchmarks) for detailed scaling data.
...
Yes, all three:
- **Python** — Via FFI with `ctypes`. Load the shared library, call the 27 exported functions. See [Integration Guide](/docs/en/planning-integration-guide).
- **JavaScript** — Via WASM. Install `@agentra/planning` from npm. Works in Node.js and browsers.
