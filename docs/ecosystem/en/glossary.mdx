...
**Temporal entity** — The atomic unit of temporal reasoning. Five types: `Duration` (time spans with PERT estimation), `Deadline` (hard or soft due dates with consequences), `Schedule` (recurring events with cron-style patterns), `Sequence` (ordered multi-step workflows), and `Decay` (values that fade over time).

**PERT estimation** — Program Evaluation and Review Technique for three-point duration estimates. Takes optimistic, expected, and pessimistic values to compute a weighted average: (O + 4M + P) / 6.

**Temporal debt** — The compounding cost of deferred work. Like financial debt that accrues interest, deferred tasks grow more expensive over time. Tracks principal, rate, and accumulated penalty.

**Chrono-gravity** — A priority warping model where approaching deadlines pull nearby tasks into their orbit, automatically escalating urgency based on proximity and consequence severity.

**Timeline fork** — A branch-and-explore mechanism for temporal reasoning. Clones the current temporal state so the agent can simulate different scheduling decisions and compare outcomes before committing.

**Temporal anomaly** — An impossible or contradictory temporal state: deadlines before their start dates, infinite sequence loops, or schedules that conflict with their own recurrence patterns.

**.atime** — The AgenticTime artifact file. A portable binary file containing all temporal entities (deadlines, schedules, sequences, durations, decay models) with BLAKE3 checksums for integrity verification.
