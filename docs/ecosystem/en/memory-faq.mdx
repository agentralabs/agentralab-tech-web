...
description: "Vector databases (Pinecone, Weaviate, Qdrant, Chroma, etc.) solve one problem: find the most similar vectors to a query. They are retrieval systems. You put embeddings in, you g..."
---
...
See the [Integration Guide](/docs/en/memory-integration-guide) for detailed examples.
...
**Rust CLI:** Build from source with `cargo install agentic-memory-cli` or `cargo build --release`. All tests pass on Windows. Memory-mapped I/O uses `CreateFileMapping`/`MapViewOfFile` under the hood.
