---
title: "API Reference"
description: "AgenticCognition exposes its functionality through four primary surfaces: the Rust library API, the MCP server, the CLI, and C FFI bindings."
---
...
All 14 MCP tools are accessible through `agentic-cognition-mcp` over JSON-RPC 2.0 stdio transport. See [MCP Tools](/docs/en/cognition-mcp-tools) for full parameter tables and response formats.
...
The `acog` binary provides 40+ commands organized into groups. See [CLI Reference](/docs/en/cognition-cli-reference) for complete command documentation.
...
The `agentic-cognition-ffi` crate exposes a C-compatible FFI surface. See [FFI Reference](/docs/en/cognition-ffi-reference) for the complete header file and memory management rules.
