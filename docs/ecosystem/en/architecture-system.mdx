...
description: "This page shows how Agentra orchestration and the five sisters operate together across local and server runtime environments."
---
...
2. `agentra` coordinates sister runtimes (`agentic-codebase-mcp`, `agentic-memory-mcp`, `agentic-vision-mcp`, `agentic-identity-mcp`, `agentic-time-mcp`).
...
2. **Sister runtimes**: `agentic-codebase-mcp`, `agentic-memory-mcp`, `agentic-vision-mcp`, `agentic-identity-mcp`, and `agentic-time-mcp` expose MCP tools.
3. **Artifacts**: `.acb` (code graph), `.amem` (memory graph), `.avis` (visual memory), `.aid` (identity anchor), `.atime` (temporal state).
