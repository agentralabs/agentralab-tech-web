...
The `.acon` binary format with `ACON` magic bytes (0x41434F4E) and BLAKE3 checksums for integrity verification. See the [file format documentation](/docs/zh/contract-file-format) for the complete specification.
...
Yes. 安装 via the auto-installer or manually add the MCP configuration:
