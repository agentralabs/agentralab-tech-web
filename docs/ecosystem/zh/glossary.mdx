---
title: "术语表"
description: "Agentra Labs 文档和 sister 项目中使用的关键术语。"
---

Agentra Labs 项目中使用的关键术语。
...
**Artifact** — A portable binary file produced by a sister. Artifacts store all state and can be moved between machines. Formats: `.amem` (memory), `.avis` (vision), `.acb` (codebase), `.aid` (identity), `.atime` (time), `.acon` (contract), `.acomm` (comm), `.aplan` (planning).
...
**运行time mode** — How a sister is deployed: `local` (single machine), `desktop` (MCP client like Claude Desktop), or `server` (remote host with auth and artifact sync).
...
**Auto-capture** — Automatic recording of prompts and feedback into memory nodes. 模式: `safe` (templates and explicit feedback), `full` (broader tool input), `off`.
...
**Channel** — A named communication pathway between agents. Channels can be direct (1:1), group, broadcast, or pub/sub. Each channel has configurable persistence, encryption, and TTL.

**Message** — The atomic unit of communication. Each message has a sender, content, type (text, command, event, data), priority, and optional threading via parent references.

**Pub/sub** — Publish-subscribe messaging where agents subscribe to topics and receive messages published to those topics without direct coupling.

**.acomm** — The AgenticComm artifact file. Stores channels, messages, subscriptions, consent records, and routing configuration.

## AgenticPlanning

**Goal** — A persistent objective with status tracking, priority, stakeholders, and relationships to other goals. Goals form the top-level planning construct.

**Decision** — A recorded choice with reasoning, alternatives considered, and traceability to goals. Decisions can be linked to stakeholders and commitments.

**Commitment** — A promise or obligation tied to a decision or goal, with deadline tracking and fulfillment status.

**Decision path** — A sequence of decisions forming a strategic trajectory. Paths can branch, merge, and be analyzed for counterfactual outcomes.

**.aplan** — The AgenticPlanning artifact file. Stores goals, decisions, commitments, stakeholder records, and strategic reasoning chains.
