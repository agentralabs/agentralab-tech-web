...
**Policy** — A governance rule that allows, denies, or requires approval for an agent action. Policies are scoped to global, session, or individual agents with tag-based matching.

**Risk limit** — A quantitative boundary on agent behavior: rate limits, threshold limits, budget limits, or count limits. The agent checks limits before acting.

**Approval workflow** — A multi-party process where controlled actions require explicit approval from designated approvers before execution.

**Obligation** — A task an agent must fulfill within a deadline. Obligations track status transitions from pending to fulfilled or overdue.

**Violation** — A recorded instance of an agent breaking a policy or exceeding a risk limit. Violations have severity levels: info, warning, critical, fatal.

**Self-healing contract** — A contract that automatically adapts its policies when violations are detected, tightening rules or adding monitoring without manual review.

**.acon** — The AgenticContract artifact file. Stores policies, risk limits, approval workflows, obligations, conditions, and violation history in a portable binary format with BLAKE3 checksums.
