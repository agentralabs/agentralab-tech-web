...
See [concepts.md](/docs/zh/planning-concepts) for the domain model. Key request/response types:
...
- `ReincarnationUpdates` — 可选 overrides for reincarnated goal
- `MergeReport` — Result of workspace merge (counts per entity type)

See [ffi-reference.md](/docs/zh/planning-ffi-reference) for the C FFI interface.
