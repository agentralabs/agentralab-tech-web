---
title: "Rust API 参考"
description: "AgenticMemory 是用 Rust 实现的。本文档涵盖核心库 API 和 `amem` CLI 工具。"
---
...
**性能：** 297 s @ 100K 节点（离线层）。在 10K 节点下，&lt;3 秒内完成。
...
**性能：** 229 s @ 100K 节点（离线层）。在 10K 节点下，&lt;3 秒内完成。
...
| `--confidence &lt;FLOAT&gt;` |置信度得分，0.0 到 1.0。默认值：1.0。 |
| `--session &lt;INT&gt;` |会话 ID。默认值：自动分配。 |
...
| `--weight &lt;FLOAT&gt;` |边权重，0.0 到 1.0。默认值：1.0。 |
...
| `PATH` | `.amem` 文件的路径。 |
| `START_ID` |起始节点ID。 |
| `--depth &lt;INT&gt;` |最大遍历深度。默认值：3。
| `--edge-type &lt;TYPE&gt;` | Filter to specific edge types.可重复。 |
| `--json` |输出为 JSON。 |
...
| `--session &lt;INT&gt;` |按会话 ID 过滤。 |
| `--search &lt;TEXT&gt;` | Semantic similarity search query. |
| `--top-k &lt;INT&gt;` |相似性搜索的结果数。默认值：10。
| `--min-confidence &lt;FLOAT&gt;` |最小置信度阈值。 |
| `--json` |输出为 JSON。 |
...
|-----------------|-------------|
| `PATH` | `.amem` 文件的路径。 |
| `--port &lt;INT&gt;` |要侦听的端口。默认值：3100。
| `--host &lt;ADDR&gt;` |要绑定的主机地址。默认值：`127.0.0.1`。 |
