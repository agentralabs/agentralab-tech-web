...
| `description` | `Option<String>` | no | `None` | 可选 human-readable description. Max 1024 characters. |
| `tags` | `Vec<String>` | no | `[]` | 可选 tags for categorization. Each tag max 64 characters. |
...
| `filter` | `Option<MessageFilter>` | no | `None` | 可选 additional filter applied to matched messages. |
