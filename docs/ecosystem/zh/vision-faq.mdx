...
No. CLIP inference runs on CPU via ONNX 运行time with a single thread. If no ONNX model is installed, Vision operates in fallback mode with zero-vector embeddings. All features except semantic similarity continue to work.
