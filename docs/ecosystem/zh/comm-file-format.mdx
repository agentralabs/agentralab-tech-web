...
Each message is a bincode-serialized struct with the fields defined in [Data Structures](/docs/zh/comm-data-structures). Key considerations:
...
- 可选 fields use bincode's `Option` encoding (1 byte tag: 0=None, 1=Some followed by value).
...
Each subscription record follows the Subscription struct layout from [Data Structures](/docs/zh/comm-data-structures).
