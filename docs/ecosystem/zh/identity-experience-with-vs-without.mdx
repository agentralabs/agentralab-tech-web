...
Create identity anchors for existing agents. No behavioral changes required — agents gain a persistent, verifiable identity but continue operating as before. 安装 the MCP server and run `identity_create` for each agent.
